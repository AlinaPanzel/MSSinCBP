function Table = behavioral_analysis(d, Table, type)
...
elseif strcmpi(type, 'correlation')
    ...
    title('CLBP')
    Table.corr.clbp.R = R;
    Table.corr.clbp.Pval = PValue;
    [h, crit_p, adj_ci_cvrg, adj_p]=fdr_bh(PValue); % FDR correction
    Table.corr.clbp.adj_Pval = adj_p;
    ...
    title('HC')
    Table.corr.hc.R = R;
    Table.corr.hc.Pval = PValue;
    [h, crit_p, adj_ci_cvrg, adj_p] = fdr_bh(PValue); % FDR correction
    Table.corr.hc.adj_Pval = adj_p;

    % Plot significant correlations
    plot_correlation(d,1, 'HC');
    plot_correlation(d,1, 'CLBP');
    plot_correlation(d,1, 'CLBP_clinical');
